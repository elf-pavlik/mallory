...
const root = signal<string | undefined>(undefined)
...
export class MalloryApp extends SignalWatcher(LitElement) {
  ...
  async connectedCallback() {
    ...
    if (session.get().webId) {
      try {
        const result = await rimraf(session.get())
        root.set(result)
      } catch (err) {
        console.error(err)
        abort.set(true)
    ...
    return abort.get()
      ...
      : !session.get().isActive
        ...
        : !root.get()
          ...
          : html`<mallory-summary></mallory-summary>`
  }
}
...
@customElement('mallory-summary')
export class MallorySummary extends LitElement {
  static styles = css`
    :host {
      background: var(--_container-color);
    }
...
export class SolidSignin extends LitElement {
  ...
  render() {
    return html`
      <button class="signin-button" @click=${login}>
        <svg
          width="20"
          height="20"
          viewBox="0 0 352 322"
